...
def setup(config):
    ...
    for ctrl in CONTROLLERS:
        CgroupCli.set(config, PARENTCG, 'cgroup.subtree_control',
                      '-{}'.format(ctrl), ignore_systemd=True)
    CgroupCli.set(config, PARENTCG, 'cgroup.subtree_control',
                  '+{}'.format(CONTROLLERS[0]), ignore_systemd=True)
    CgroupCli.get_and_validate(config, PARENTCG, 'cgroup.subtree_control', CONTROLLERS[0], True)
