 ...
 */
static void cgroup_exp_scope_path(const char *const slice_name, const char *const scope_name,
				 char *exp_scope_path, bool *expanded)
{
	...
	*expanded = false;
	...
	snprintf(exp_scope_path + offset, exp_slice_len - offset, "%s/%s", slice_name, scope_name);
			 ...
			 const struct cgroup_systemd_scope_opts * const opts)
{
	 ...
	 */
	cgroup_exp_scope_path(slice_name, scope_name, exp_scope_path, &expanded);
	if (expanded)
